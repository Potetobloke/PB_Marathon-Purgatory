class ZWeapon : Weapon
{
    ...
    */
    ...
    action void ZWL_WeaponReady(int flags = 0)
    {
        ...
        // If NoAutofire
        wrf |= flags & ZRF_NoPrimary || !invoker.CheckMagazine(false) || !invoker.JustPressed(BT_Attack) && !invoker.bNoAutofire ? WRF_NoPrimary : 0;
        ...
        wrf |= !(flags & ZRF_NoPartialReload) && !invoker.IsFull(flags & ZRF_ExtraRound)
            && (invoker.CheckMagazine(false) || !(flags & ZRF_NoReload))
    ...
    */
    virtual bool CheckMagazine(bool autoReload, bool requireAmmo = true, int ammoCount = -1)
    {
        if (magazineSize <= 0)
